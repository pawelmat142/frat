...
export class WizardService implements OnModuleInit {
  ...
  private async onBotMessage(message: TelegramBot.Message) {
    ...
    if (!message.from.username) {
      this.logger.error('Telegram username not found');
      await this.sendMessage(chatId, 'Error: Telegram username not found.');
      return;
    }
    ...
    let wizard = await this.findOrCreateWizard(chatId, message.from.username!);
  ...
  private async findOrCreateWizard(chatId: number, telegramUsername: string): Promise<Wizard> {
    this.showTyping(chatId);
    let wizard = this.findWizard(chatId);
    if (!wizard) {
      wizard = await this.prepareWizard(chatId, telegramUsername);
    }
  ...
  public async startNewWizard(chatId: number, telegramUsername: string) {
    let wizard = await this.findOrCreateWizard(chatId, telegramUsername);
  ...
  private async prepareWizard(chatId: number, telegramUsername: string): Promise<Wizard> {
    ...
    const wizard: Wizard = user
      ...
      : new NewUserWizard(chatId, telegramUsername, this.servicesProvider);
